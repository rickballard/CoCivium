...
- topic priority buckets (pick one): ["post-capture government recovery","transparent standards adoption","trust & provenance","civic safety drills","education/onboarding","workflow integrity","policy-as-code"]
- do NOT propose implementation; propose at IdeaCard fidelity
...
async function run(){
  ...
  if(!resp.ok){ console.error("OpenAI error:", resp.status, await resp.text()); process.exit(1); }
  const data = await resp.json();
  const raw = data?.choices?.[0]?.message?.content?.trim();
  const jsonish = extractJson(raw);
  if (!jsonish){ console.error("Non-JSON AI output:", raw); process.exit(1); }
