param([Parameter(Mandatory=$true)][string]$MasterCsv,[Parameter(Mandatory=$true)][string]$LinkedInCsv)
...
foreach($l in $linkedin){
  ...
  if($em){$em=$em.Trim().ToLower();if(-not $byEmail.ContainsKey($em)){$byEmail[$em]=@()};$byEmail[$em]+=$l}
}
...
foreach($r in $master){
  ...
  if($r.email -and $byEmail.ContainsKey($r.email)){
    ...
    $cand=$linkedin | Where-Object{
      ($_. "First Name" + ' ' + $_. "Last Name") -and ($r.full_name) -and ($r.full_name.Split()[0]) -eq ($_. "First Name")
