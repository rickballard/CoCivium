...
function Render-Line($it){
  ...
  $linksStr = $links -join " • "
  return "> :sparkles: **New**$dateBits: {0} → {1}" -f $it.title, $linksStr
}
...
if($readme -match '<!-- NEWLIST:BEGIN -->.*?<!-- NEWLIST:END -->'){
  $readme = [regex]::Replace($readme, '<!-- NEWLIST:BEGIN -->.*?<!-- NEWLIST:END -->', [System.Text.RegularExpressions.MatchEvaluator]{ param($m) $blockText }, 'Singleline')
}else{
  # Insert after the first line (usually the H1), otherwise prepend.
  if($readme -match '^\# .*$'){
    $readme = [regex]::Replace($readme, '(^\# .*$\r?\n?)', { param($m) $m.Value + "`n" + $blockText + "`n`n" }, 'Multiline')
  }else{
