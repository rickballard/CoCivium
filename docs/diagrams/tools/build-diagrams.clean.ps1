...
$warn = @()
foreach($n in $nodes){
  if([string]::IsNullOrWhiteSpace($n.id)){ $warn += "node missing id: $($n | ConvertTo-Json -Compress)" }
}
foreach($e in $links){
  if([string]::IsNullOrWhiteSpace($e.source) -or [string]::IsNullOrWhiteSpace($e.target)){
    $warn += "link missing source/target: $($e | ConvertTo-Json -Compress)"
  }
}
if($warn){
  "`n[diagram-lint] warnings:" | Write-Host
  $warn | % { " - param([string]$RepoRoot = (Resolve-Path (Join-Path $PSScriptRoot '..\..\..')).Path)
$ErrorActionPreference='Stop'; Set-StrictMode -Version Latest
