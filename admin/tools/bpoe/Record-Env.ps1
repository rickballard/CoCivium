# Record-Env.ps1 — capture local OE/BPOE environment into admin/history (v0.1)
[CmdletBinding()]
param()
$ErrorActionPreference='Stop'
function TryVer($name, $cmd, $args='--version'){
  try{
    $p = Start-Process -FilePath $cmd -ArgumentList $args -NoNewWindow -PassThru -RedirectStandardOutput "STDOUT.tmp" -RedirectStandardError "STDERR.tmp" -Wait
