...
foreach($f in $files){
  try{
    ...
    foreach($k in @('budget','window','rollback_threshold')){
      if($null -eq $obj.$k){ $missing += "$($f.FullName): missing $k" }
    }
  } catch { $missing += "$($f.FullName): read/parse failed: $($_.Exception.Message)" }
}
if($missing.Count){ "Missing fields:"; $missing | % { " - $_" }; exit 2 }
